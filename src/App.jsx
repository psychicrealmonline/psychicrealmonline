...
function AuthScreen({ onEnter }) {
  ...
  async function register() {
    ...
    try {
      ...
      if (authData.access_token) {
        onEnter({ userId:uid, username, xp:0, totalCorrect:0, badges:[], bestStreak:0 });
      } else {
        setInfo("Account created! Check your email to confirm, then sign in.");
        switchMode("login");
      }
  ...
  async function login() {
    ...
    try {
      ...
      const stats  = await sb.select("player_stats", `player_id=eq.${uid}&select=xp,total_correct,badges,best_streak`, true);
      onEnter({ userId:uid, username:player.username, xp:stats?.xp||0, totalCorrect:stats?.total_correct||0, badges:stats?.badges||[], bestStreak:stats?.best_streak||0 });
    } catch(e) {
...
export default function PsychicMMORPG() {
  ...
  function handleEnter({ userId, username, xp:sx, totalCorrect:sc, badges:sb2, bestStreak:sbs }) {
        ...
        {card&&(
          <div style={{width:"100%",maxWidth:680,background:"linear-gradient(160deg,rgba(7,16,34,0.9),rgba(15,30,64,0.9))",backdropFilter:"blur(20px)",border:"1px solid #1a3a6a",borderRadius:24,padding:"2rem",boxShadow:"0 0 60px rgba(20,60,150,0.3)",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:"0.8rem",flexWrap:"wrap"}}>
              {SYMBOLS.map(sym=>(
                <button key={sym.id} onClick={()=>handleGuess(sym)} disabled={revealed}
                  style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0.4rem",padding:"0.9rem 1.1rem",borderRadius:14,cursor:revealed?"not-allowed":"pointer",
                    ...
                    color:"#c8d8f0",transition:"all 0.2s",opacity:revealed?0.7:1}}>
